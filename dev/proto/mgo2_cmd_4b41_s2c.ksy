meta:
  id: mgo2_cmd_4b41_s2c
  title: "MGO2 0x4b41 — cancel join / leave clan result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Cancel Join, or leave the clan.** The reply to 0x4b40, which carries **no payload at all** —
  which is why it never turned up in a search for id-carrying commands.
  [CONFIRMED LIVE 2026-07-27]: it is what the pending-applicant screen's "Cancel Join" button
  sends, identified by answering it with a bare result and watching the application stay in place
  until the server actually withdrew it.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD56A9C**,
  which re-checks the id (`cmpwi r0,19265` = 0x4b41) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 97
  ...
  On success the parser makes THREE further calls to 0xD3A094 (a session-object getter)
  before closing the slot; those touch client state, not the wire.
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Withdraw result. 0 = the application, or the membership, was dropped.
