...
doc: |
  ...
  1. fetches the inbound packet (`0xD3879C`) and checks `hdr.command == 0x43A1`
  ...
  3. `0xD32E08(ctx, 45, 2)` — mark request-status slot **45** complete
     (slot table at `ctx+360+4*id`, ids capped at 116);
  4. `0xD32E70(ctx, 45, result)` — store the u32 as that slot's result
...
seq:
  - id: result
    type: s4
    doc: |
      [ELF 0xd4046c] The only field. 0 = success. Signed: the client stores it with `lwa`
