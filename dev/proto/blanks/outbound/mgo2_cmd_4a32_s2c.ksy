meta:
  id: mgo2_cmd_4a32_s2c
  title: "MGO2 0x4A32 - unmapped clan/GHQ-block reply (server -> client)"
  endian: be
doc: |
  UNMAPPED SUBSYSTEM. Nothing in dev/docs/PROTOCOL.md or dev/docs/OBSERVED.md describes
  0x4A32; COMMANDS.md lists it only as "parsed but never sent". Everything below is read out of
  the client parser - field ORDER and WIDTH are solid, MEANINGS are not.

  Evidence: dispatcher 0xD38804 (the 0x41xx-0x4Exx literal compare chain), entry stub 0xD39910,
  parser 0xD52758.

  Followed by a memset (0xDD36F8) - a list-reset, as with 0x4A10.
