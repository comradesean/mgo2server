meta:
  id: mgo2_cmd_4399_s2c
  title: "MGO2 0x4399 — server -> client: bare result ack for 0x4398 update pings"
  endian: be
doc: |
  Evidence: reply dispatcher `0xD38804` (the `0x41xx`-`0x4Exx` compare chain) matches
  `cmpwi 0x4399` and branches to the stub at `0xd392b0`, which tail-calls the parser at
  `0xd40530`.
  ...
  1. fetches the inbound packet (`0xD3879C`) and checks `hdr.command == 0x4399`
  ...
  3. `0xD32E08(ctx, 44, 2)` — mark request-status slot **44** complete
     (slot table at `ctx+360+4*id`, ids capped at 116);
  4. `0xD32E70(ctx, 44, result)` — store the u32 as that slot's result
  ...
  **Contradiction with PROTOCOL.md.** PROTOCOL.md says "Reply stays the **empty** 0x4399
  that is live-verified". The parser at 0xd40530 unconditionally reads a u32 (0xd5cc64)
  and bails to -71 without signalling status 44 if that read fails. An empty payload only
  works because the read primitives bound-check against the **1023-byte receive buffer**,
  not against the payload length (0xd5cc64: `cursor > 1020` is the only test) — so the
  client reads four bytes of whatever the buffer already held. It is a live-verified
  accident, not a documented empty reply. Sending an explicit 4-byte zero is strictly
  safer and is what this spec describes.
doc-ref: dev/docs/COMMANDS.md, dev/docs/PROTOCOL.md
seq:
  - id: result
    type: s4
    doc: |
      [ELF 0xd40530] The only field. 0 = success. Signed: the client stores it with `lwa`
