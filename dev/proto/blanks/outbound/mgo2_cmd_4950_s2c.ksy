meta:
  ...
  encoding: ISO-8859-1
doc: |
  Evidence: reply dispatcher `0xD38804` matches `cmpwi 0x4950` at `0xd38c90` and branches to the
  ...
  bytes write member+0x11/+0x12; additionally the slot whose `character_id` matches the local
  ...
  The 204-byte block's layout is modelled once, in `mgo2_cmd_4909.ksy` (`block_204`); it is
  kept opaque here so the two files cannot drift.
...
seq:
  ...
  - id: block_204
    ...
    doc: |
      ...
      `mgo2_cmd_4909.ksy`, type `block_204`.
