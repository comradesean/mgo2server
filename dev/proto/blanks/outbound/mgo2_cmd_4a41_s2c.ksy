meta:
  id: mgo2_cmd_4a41_s2c
  title: "MGO2 0x4A41 - unmapped clan/GHQ-block reply (server -> client)"
  endian: be
doc: |
  UNMAPPED SUBSYSTEM. Nothing in dev/docs/PROTOCOL.md or dev/docs/OBSERVED.md describes
  0x4A41; COMMANDS.md lists it only as "parsed but never sent". Everything below is read out of
  the client parser - field ORDER and WIDTH are solid, MEANINGS are not.

  Evidence: dispatcher 0xD38804 (the 0x41xx-0x4Exx literal compare chain), entry stub 0xD39940,
  parser 0xD4F498.

  Raises TWO completion events (0xD32E70 twice) where the others raise one.
