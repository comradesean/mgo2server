meta:
  id: mgo2_cmd_4924_s2c
  title: "MGO2 0x4924 — server -> client: result ack, 0x49xx clan/GHQ family"
  endian: be
  encoding: ISO-8859-1
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (compare tree at `0xD38804`) matches `cmpwi 0x4924` at `0xd38c54` and branches to the
  thunk at `0xd396a0`, which tail-calls the parser at `0xd49c0c`. Channel A (lobby TCP).
  ...
  The parser is a bare result ack: verify `hdr.command == 0x4924` (mismatch -> `-70`, the
  no-handler code), rewind, **one** 4-byte read into a stack slot, end-of-read, then
  `0xD32E08(ctx, 67, 2)` to complete request-status slot 67 and `0xD32E70(ctx, 67, result)`
...
seq:
  - id: result
    type: s4
    doc: |
      [ELF 0xd49c0c] The only field; the whole payload is 4 bytes. 0 = success. Signed: the
