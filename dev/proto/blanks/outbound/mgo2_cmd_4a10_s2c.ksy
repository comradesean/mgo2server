meta:
  id: mgo2_cmd_4a10_s2c
  title: "MGO2 0x4A10 - unmapped clan/GHQ-block reply (server -> client)"
  endian: be
doc: |
  UNMAPPED SUBSYSTEM. Nothing in dev/docs/PROTOCOL.md or dev/docs/OBSERVED.md describes
  0x4A10; COMMANDS.md lists it only as "parsed but never sent". Everything below is read out of
  the client parser - field ORDER and WIDTH are solid, MEANINGS are not.

  Evidence: dispatcher 0xD38804 (the 0x41xx-0x4Exx literal compare chain), entry stub 0xD39880,
  parser 0xD52170.

  After RD_END the parser zeroes a block via memset (0xDD36F8) - i.e. this
  reply RESETS a client-side list before the entries arrive. No entry record is read here.
