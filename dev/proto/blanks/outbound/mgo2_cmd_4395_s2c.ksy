meta:
  id: mgo2_cmd_4395_s2c
  title: "MGO2 0x4395 — server -> client: bare result ack for 0x4394 (never observed; COMMANDS.md lists it as a reachable gap, large struct)"
  endian: be
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (the `0x41xx`-`0x4Exx` id space; compare tree head at `0xD38804`) matches
  `cmpwi 0x4395` and branches to the stub at `0xd392a0`, which tail-calls the parser at
  `0xd405f4`.
  ...
  1. fetches the inbound packet (`0xD3879C`) and checks `hdr.command == 0x4395`
  ...
  3. `0xD32E08(ctx, 43, 2)` — mark request-status slot **43** complete
     (slot table at `ctx+360+4*id`, ids capped at 116);
  4. `0xD32E70(ctx, 43, result)` — store the u32 as that slot's result
  ...
  COMMANDS.md files 0x4395 under "result singles" — parsed but never sent. The parser
  confirms the shape: a single u32 result and nothing else.
...
seq:
  - id: result
    type: s4
    doc: |
      [ELF 0xd405f4] The only field. 0 = success. Signed: the client stores it with `lwa`
