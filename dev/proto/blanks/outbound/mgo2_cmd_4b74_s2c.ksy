...
doc: |
  ...
  Routing: dispatcher 0xD38804 (the 0x41xx-0x4Exx compare chain) -> thunk -> parser **0xD5502C**,
  which re-checks the id (`cmpwi r0,19316` = 0x4b74) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 112
  ...
  List-triple: the item records arrive as 0x4b75; this packet is the paired start/end.
  Gate word at session[+0x10000+6404] + 0x20000 + 29724: must be 0 on arrival; the parser stores the result into it (arming it).
