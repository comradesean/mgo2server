meta:
  ...
  title: "MGO2 0x4A33 - unmapped 0x4Axx list reply, 45-byte records (server -> client)"
  endian: be
doc: |
  UNMAPPED SUBSYSTEM. Nothing in dev/docs/PROTOCOL.md or dev/docs/OBSERVED.md describes
  0x4A33; COMMANDS.md lists the 0x49xx/0x4Axx/0x4Bxx blocks only as "parsed but never sent".
  Field ORDER and WIDTH below come out of the client parser and are solid. MEANINGS are not.

  Evidence: dispatcher 0xD38804 (the 0x41xx-0x4Exx literal compare chain), entry stub 0xD39920,
  parser 0xD524F4.
  ...
  client also caps insertion against a stored u16 at obj+0x0D6 (0xD526C8), so records past
  that cap are parsed and dropped rather than rejected.

  READ SEQUENCE IS IDENTICAL TO 0x4A11 (parser 0xD51F2C) field for field. Separate functions,
...
seq:
  - id: entries
    type: entry
    repeat: eos
    doc: "[ELF] size-driven (0xD525B4). 45 bytes each; a short record desyncs the rest of the packet rather than erroring."
types:
  entry:
    ...
    seq:
      - id: unknown_0x00
        type: u2
        doc: "[UNKNOWN] read FIRST, at 0xD525D0 (-> r1+114), before the word. Do not reorder."
      - id: unknown_0x02
        type: u4
        doc: "[UNKNOWN] read at 0xD525E8 -> record+0x00."
      - id: name
        size: 16
        type: str
        encoding: ISO-8859-1
        pad-right: 0
        doc: "[INFERRED] 16-byte raw read (0xD52608) -> record+0x04. Width certain; string role from the width only."
      - id: unknown_0x16
        type: u1
        doc: "[UNKNOWN] read at 0xD52624 -> record+0x15."
      - id: unknown_0x17
        type: u1
        doc: "[UNKNOWN] read at 0xD52640 (-> r1+112)."
      - id: name2
        size: 16
        type: str
        encoding: ISO-8859-1
        pad-right: 0
        doc: "[INFERRED] second 16-byte raw read (0xD52678) -> record+0x18."
      - id: unknown_0x28
        type: u1
        doc: "[UNKNOWN] read at 0xD52694 -> record+0x29."
      - id: unknown_0x29
        type: u4
        doc: "[UNKNOWN] read at 0xD526B0 -> record+0x2C. Last field of the record."
