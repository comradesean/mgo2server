meta:
  ...
  title: "MGO2 0x4b30 — clan/GHQ request (client -> server)"
  endian: be
doc: |
  ...
  (`li r4,0x4b30` in the preceding instruction); the ONLY payload write between the
  ...
  Preconditions: session != NULL, and 0xD5709C true — the session clan record must exist
  (id != 0) with status byte == 2. Otherwise the sender returns -1203 and sends nothing.

  On a successful flush the client advances its flow state with
  `0xD32E08(session, 94, 1)`. The sender does not name the reply id.
