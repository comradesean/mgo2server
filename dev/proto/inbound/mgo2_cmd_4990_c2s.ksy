...
doc: |
  ...
  **Payload is Blowfish-encrypted** (PROTOCOL.md, DECRYPT_COMMANDS) — of nothing, see below.
  Reply `0x4991`, 172 bytes.

  **Zero payload bytes:** the finaliser `0xD5C828` is called at `0xD47AA0`, one instruction after
  the builder returns, so the length word is 0 and the encrypted body is empty. This is the
  send-side confirmation of PROTOCOL.md's "request payload is not read".
