...
doc: |
  ...
  (`li r4,61` at `0xD4A7F0` region). Unhandled.

  **Zero payload bytes:** the finaliser `0xD5C828` is called at `0xD4A7D8`, one instruction after
  the builder returns, with no primitive in between.
