meta:
  id: mgo2_cmd_4b49_s2c
  title: "MGO2 0x4b49 — clan emblem for your own clan, result + 768 bytes (server -> client)"
  endian: be
doc: |
  ...
  **This id is 0x4b48's reply.** 0x4b48 is sent unprompted at login by any character who IS in a
  clan — the payload is the client's own cached clan id, read straight out of session_ctx+0x1AA0
  rather than passed in (0xD577A4). It appeared the moment 0x4122 first reported a real clan, and
  it **blocks: character select hangs without a reply.** [CONFIRMED LIVE 2026-07-27]

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser
  **0xD56F24**, which re-checks the id (`cmpwi r0,19273`) before reading anything.

  Closes pending-request slot **101**. Destination buffer is memset to 0 over 769 bytes first
  ...
  Error-code laundering (0xD5701C-0xD57068, unique to this id): results -21, -1207, -1215 and
  -1202 are all REWRITTEN to -1214 before being published, and -1214 additionally sets a state
  byte to 2 in the object returned by 0xD56EDC. So the client collapses four distinct server
  errors into one user-visible one; do not rely on the server's code reaching the UI.
...
seq:
  - id: result
    type: s4
    doc: "[CONFIRMED 2026-07-27] 0 = success and the 768-byte emblem follows; non-zero makes this a 4-byte reply. Published to request slot 101."
  - id: emblem
    size: 768
    doc: |
      [CONFIRMED 2026-07-27] The clan's emblem, 768 bytes, read as one fixed block (0xD5D018,
      len=0x300) and copied to profile+6873. Opaque to the parser — it decodes not one byte — and
      the client NUL-terminates at +768, hence the 769-byte destination. The internal pixel/format
      encoding is [UNKNOWN]; the server never needs it, since the client both produces (0x4b50) and
