meta:
  ...
  title: "MGO2 0x491a — server -> client: team notification — header only (no payload beyond the 6-byte key)"
  endian: be
  encoding: ISO-8859-1
doc: |
  ...
  local team-state teardown `0x4915` runs — and then raises event 3. So the whole payload is
  the 6-byte key, and its effect is "your cached team is gone".
  ...
  **RELABELLED 2026-08-01 — the header pair is the TEAM id and the TEAM record serial, not a
  ...
  record: this parser re-bases it with `addis` +1 / `addi ...,-9944` at [ELF 0xd4d374] and passes
