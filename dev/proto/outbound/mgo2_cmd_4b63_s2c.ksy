meta:
  ...
  title: "MGO2 0x4b63 — set clan emblem editor result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Assign emblem-editing rights to a member.** The reply to 0x4b62,
  `{u32 target character id}`, leader only. [CONFIRMED LIVE 2026-07-27].

  The assignee's character id is what the server then reports at 0x4b21's **T+0x6FC**: the client
  compares that field against its own character id to decide whether to offer "set as the clan's
  emblem". No bit of the privilege word is involved — see mgo2_cmd_4b47_s2c.ksy.
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Set-emblem-editor result. 0 = the named member may now edit the emblem.
