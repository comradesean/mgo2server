meta:
  id: mgo2_cmd_4b93_s2c
  title: "MGO2 0x4b93 — clan search END (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **End of the clan search results.** Last packet of the 0x4b91 / 0x4b92 / 0x4b93 triple answering
  0x4b90. [CONFIRMED LIVE 2026-07-27].

  **This parser is the corroborating evidence for 0x4b11's header word order.** It fills the same
  two list-object slots the clan list header fills from the wire, and it fills them itself: it sets
  block+0x08 = **0** and block+0x0C = **the record count** (0xD54D64, 0xD54D78). So block+0x08 is
  the page offset and block+0x0C is the total — which is what 0x4b11 must send as {offset, total},
  in that order. See mgo2_cmd_4b11_s2c.ksy.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD54CB0**,
  which re-checks the id (`cmpwi r0,19347` = 0x4b93) before reading anything.
  ...
  Gate word at session[+0x10000+6404] + 0x20000 + 10344: must be NON-zero on arrival; the parser copies the record count from +0x04 to +0x0c.
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Search-end result. 0 = the results are complete. **A result code,
      never a count** — the count the client uses is the one this parser derives itself, see the
      top-level doc.
