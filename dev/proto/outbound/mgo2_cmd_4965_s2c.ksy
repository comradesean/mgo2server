meta:
  id: mgo2_cmd_4965_s2c
  title: "MGO2 0x4965 — server -> client: team notification carrying one 4-byte word"
  endian: be
  encoding: ISO-8859-1
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (compare tree at `0xD38804`) matches `cmpwi 0x4965` at `0xd38ca8` and branches to the
  thunk at `0xd397b0`, which tail-calls the parser at `0xd4c4ac`. Channel A (lobby TCP).
  ...
  request-status slot. The parser ends by raising client event **13** via `0xD33CD8(ctx, 13)`.
  ...
  writes bytes at member+0x12 and member+0x11 [READ 0xd4c574-0xd4c5bc]. Payload 10 bytes;
  event 13.
  ...
  **RELABELLED 2026-08-01 — the header pair is the TEAM id and the TEAM record serial, not a
  ...
  record: this parser re-bases it with `addis` +1 / `addi ...,-9944` at [ELF 0xd4c4e8] and passes
...
seq:
  ...
  - id: character_id
    type: u4
    doc: "[INFERRED] used to locate a member slot whose state bytes at +0x11/+0x12 are then rewritten."
