meta:
  id: mgo2_cmd_4b61_s2c
  title: "MGO2 0x4b61 — transfer clan leadership result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Hand leadership to another member.** The reply to 0x4b60, `{u32 target character id}`, leader
  only. [CONFIRMED LIVE 2026-07-27] — the whole payload is one result word.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD552E4**,
  which re-checks the id (`cmpwi r0,19297` = 0x4b61) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 106
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Transfer-leadership result. 0 = the named member is now the leader.
