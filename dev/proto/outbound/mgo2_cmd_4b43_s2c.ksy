meta:
  id: mgo2_cmd_4b43_s2c
  title: "MGO2 0x4b43 — apply to join clan result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Apply to join a clan.** The reply to 0x4b42, `{u32 clan id}`. [CONFIRMED LIVE 2026-07-27].

  Two client-side facts that belong with this id:
    * 0x4b42's sender **refuses to transmit** unless the session clan record at
      session_ctx+0x1AA0 holds a non-zero id — it returns -24 (0xFFFFFFE8) having sent nothing.
      That record is filled by 0x4b81, so a zero-filled 0x4b81 makes Apply fail with -24 and no
      packet on the wire at all.
    * After sending, the client writes the clan id into its own record with **status 0, pending**
      (0xD58714), leaving 1 (member) and 2 (leader) to come from the server. The application is
      therefore a real state on both sides.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD56BE0**,
  which re-checks the id (`cmpwi r0,19267` = 0x4b43) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 96
  ...
  On success the parser makes ONE further call to 0xD3A094 before closing the slot.
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Apply-to-join result. 0 = the application was recorded.
      ...
      A negative value is one of the CLIENT's own error codes, not a server-invented one: the
      client resolves it through the table at 0x106D714 (664 entries of {u32 dialogId, i32
      ordinal}, scanned by 0xB8F988, raised by 0x885A08, resolved via 0x240708) into a dialog
      string. See dev/docs/PROTOCOL.md.
