meta:
  id: mgo2_cmd_4b37_s2c
  title: "MGO2 0x4b37 — banish member result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Banish a member.** The reply to 0x4b36, `{u32 target character id}`, leader only.
  [CONFIRMED LIVE 2026-07-27] — the whole payload is one result word.

  Note this id was for a while answered from an acknowledge-only table that returned success
  without doing anything, so banish reported working while the member stayed. That table is gone;
  a bare success here now means the membership really was removed.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD5515C**,
  which re-checks the id (`cmpwi r0,19255` = 0x4b37) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 108
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Banish result. 0 = the named member was removed from the clan.
