meta:
  ...
  title: "MGO2 0x4961 — server -> client: team notification carrying one 4-byte word"
  endian: be
  encoding: ISO-8859-1
doc: |
  ...
  Header, then a single 4-byte read stored at **team+0x268** — the game-id slot of the team
  record's trailer. Post-processing walks the eight member slots resetting their state bytes (`0` and
  ...
  **RELABELLED 2026-08-01 — the header pair is the TEAM id and the TEAM record serial, not a
  ...
  record: this parser re-bases it with `addis` +1 / `addi ...,-9944` at [ELF 0xd4c744] and passes
...
seq:
  ...
  - id: game_id
    type: u4
    doc: |
      [ELF 0xd4c788] -> team+0x268, the game-id slot of the team record's trailer. The u32
      reader is handed `team+0x268` directly as its destination (`addi r4,r29,-9328`, i.e.
      `-9944 + 616`), so there is no intermediate.
