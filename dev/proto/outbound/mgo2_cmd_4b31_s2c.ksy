meta:
  id: mgo2_cmd_4b31_s2c
  title: "MGO2 0x4b31 — accept join application result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Accept a join application.** The reply to 0x4b30, `{u32 target character id}`, which only a
  leader sends. [CONFIRMED LIVE 2026-07-27] — the whole payload is one result word; the applicant
  becomes a member and the next 0x4b52 roster carries them with `is_member = 1`.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD55A40**,
  which re-checks the id (`cmpwi r0,19249` = 0x4b31) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 94
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Accept-applicant result. 0 = the application was approved.
