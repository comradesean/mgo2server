meta:
  id: mgo2_cmd_4921_s2c
  title: "MGO2 0x4921 — server -> client: result ack, 0x49xx clan/GHQ family"
  endian: be
  encoding: ISO-8859-1
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (compare tree at `0xD38804`) matches `cmpwi 0x4921` at `0xd38c48` and branches to the
  thunk at `0xd39690`, which tail-calls the parser at `0xd49cd0`. Channel A (lobby TCP).
  ...
  The parser is a bare result ack: verify `hdr.command == 0x4921` (mismatch -> `-70`, the
  no-handler code), rewind, **one** 4-byte read into a stack slot, end-of-read, then
  `0xD32E08(ctx, 66, 2)` to complete request-status slot 66 and `0xD32E70(ctx, 66, result)`
...
seq:
  - id: result
    type: s4
    doc: |
      [ELF 0xd49cd0] The only field; the whole payload is 4 bytes. 0 = success. Signed: the
