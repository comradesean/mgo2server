meta:
  id: mgo2_cmd_43c1_s2c
  title: "MGO2 0x43C1 — server -> client: bare result ack for 0x43c0 in-game info (edit name/comment/password)"
  endian: be
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (the `0x41xx`-`0x4Exx` id space; compare tree head at `0xD38804`) matches
  `cmpwi 0x43C1` and branches to the stub at `0xd392d0`, which tail-calls the parser at
  `0xd4015c`.
  ...
  1. fetches the inbound packet (`0xD3879C`) and checks `hdr.command == 0x43C1`
  ...
  3. `0xD32E08(ctx, 46, 2)` — mark request-status slot **46** complete
     (slot table at `ctx+360+4*id`, ids capped at 116);
  4. `0xD32E70(ctx, 46, result)` — store the u32 as that slot's result
  ...
  PROTOCOL.md's admin-action table maps the host's "Edit name/comment/password" action to
  0x43c0, so this ack is exercised whenever that menu is used.
...
seq:
  - id: result
    type: s4
    doc: |
      [ELF 0xd4015c] The only field. 0 = success. Signed: the client stores it with `lwa`
