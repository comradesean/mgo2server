meta:
  id: mgo2_cmd_4b4d_s2c
  title: "MGO2 0x4b4d — clan emblem, second fetch, result + 768 bytes (server -> client)"
  endian: be
doc: |
  ...
  **This id is 0x4b4c's reply** — a second clan-scoped fetch carrying the clan id, sent right after
  the profile, same shape as 0x4b49. [CONFIRMED LIVE 2026-07-27] that answering it with the emblem
  keeps the clan screens moving; [UNKNOWN] what distinguishes it from 0x4b49 beyond the request
  slot (103 vs 101).

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser
  **0xD59DA0**, which re-checks the id (`cmpwi r0,19277`) before reading anything.

  Closes pending-request slot **103**. Destination buffer is memset to 0 over 769 bytes first
...
seq:
  - id: result
    type: s4
    doc: "[CONFIRMED 2026-07-27] 0 = success and the 768-byte emblem follows; non-zero makes this a 4-byte reply. Published to request slot 103."
  - id: emblem
    size: 768
    doc: |
      [CONFIRMED 2026-07-27] The clan's emblem, 768 bytes, read as one fixed block (0xD5D018,
      len=0x300). Opaque to the parser; the client NUL-terminates at +768. The internal
      pixel/format encoding is [UNKNOWN] and the server does not need it.
