meta:
  id: mgo2_cmd_4913_s2c
  title: "MGO2 0x4913 — server -> client: team record (shared 0xD4AF34 layout)"
  endian: be
  encoding: ISO-8859-1
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (compare tree at `0xD38804`) matches `cmpwi 0x4913` at `0xd38be4` and branches to the
  thunk at `0xd39670`, which tail-calls the parser at `0xd4b5d0`. Channel A (lobby TCP).
  ...
  Wrapper `0xd4b5d0` — **polarity corrected 2026-08-03; the previous statement here was
  ...
  alone), and ONLY a `-1` return completes request-status slot **64** with the result via `0xD32E08`/`0xD32E70`.
