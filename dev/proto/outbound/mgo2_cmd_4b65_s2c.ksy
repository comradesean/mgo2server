meta:
  id: mgo2_cmd_4b65_s2c
  title: "MGO2 0x4b65 — set clan comment/description result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Set the clan comment (description).** The reply to 0x4b64, a 128-byte fixed-width text write.
  [CONFIRMED LIVE 2026-07-27]: setting **Clan Comment** to "bench" on the client sent exactly this,
  128 bytes — which is what identifies 0x4b64 as the description write rather than the notice one.
  The same text comes back at 0x4b21's T+0x67A and 0x4b81's T+0x67A, and it is the same struct
  offset 0x4b00's create request reads its own description from.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD569CC**,
  which re-checks the id (`cmpwi r0,19301` = 0x4b65) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 109
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Set-description result. 0 = the clan comment was stored.
