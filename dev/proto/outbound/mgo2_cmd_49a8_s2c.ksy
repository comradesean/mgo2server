meta:
  ...
  title: "MGO2 0x49a8 — server -> client: team notification — serial bump (u16 payload)"
  endian: be
  encoding: ISO-8859-1
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (compare tree at `0xD38804`) matches `cmpwi 0x49a8` at `0xd38d28` and branches to the
  ...
  Header, then a single u16 written to **team+0x29C** [READ 0xd4bf10] — the very field the
  header's own second word is validated against. So `0x49a8` is how the server advances the
  team record's serial: the packet must arrive carrying the *current* serial in its header and
  ...
  **RELABELLED 2026-08-01 — the header pair is the TEAM id and the TEAM record serial, not a
  ...
  record: this parser re-bases it with `addis` +1 / `addi ...,-9944` at [ELF 0xd4be78] and passes
...
seq:
  ...
  - id: new_team_serial
    type: u2
    doc: |
      [ELF 0xd4bf10] Written to team+0x29C, replacing the value that `team_serial` above had
