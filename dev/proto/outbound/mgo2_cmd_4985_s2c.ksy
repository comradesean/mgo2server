meta:
  id: mgo2_cmd_4985_s2c
  title: "MGO2 0x4985 — server -> client: team record — allocating variant (shared 0xD4AF34 layout)"
  endian: be
  encoding: ISO-8859-1
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (compare tree at `0xD38804`) matches `cmpwi 0x4985` at `0xd38cdc` and branches to the
  thunk at `0xd39650`, which tail-calls the parser at `0xd4b6b0`. Channel A (lobby TCP).
  ...
  Wrapper `0xd4b6b0` — **polarity corrected 2026-08-03; the previous statement here was
  ...
  alone), and ONLY a `-1` return completes request-status slot **63** with the result via `0xD32E08`/`0xD32E70`.
  ...
  `0x4985` additionally requires request-status slot 63 to read as 1 (pending) via
  `0xD32E3C`, else the reply is accepted but silently produces no state change
  [READ 0xd4b02c-0xd4b058]. It is the allocating variant: the 680-byte record is memset
  before the fields land.
