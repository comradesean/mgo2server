meta:
  id: mgo2_cmd_4b33_s2c
  title: "MGO2 0x4b33 — decline join application result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Decline a join application.** The reply to 0x4b32, `{u32 target character id}`, leader only —
  the mirror of 0x4b30/0x4b31. [CONFIRMED LIVE 2026-07-27] — the whole payload is one result word.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD5597C**,
  which re-checks the id (`cmpwi r0,19251` = 0x4b33) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 95
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Decline-applicant result. 0 = the application was rejected.
