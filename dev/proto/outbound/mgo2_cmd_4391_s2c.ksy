meta:
  id: mgo2_cmd_4391_s2c
  title: "MGO2 0x4391 — server -> client: bare result ack for 0x4390 update stats"
  endian: be
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (the `0x41xx`-`0x4Exx` id space; compare tree head at `0xD38804`) matches
  `cmpwi 0x4391` and branches to the stub at `0xd39280`, which tail-calls the parser at
  `0xd4077c`.
  ...
  1. fetches the inbound packet (`0xD3879C`) and checks `hdr.command == 0x4391`
  ...
  3. `0xD32E08(ctx, 41, 2)` — mark request-status slot **41** complete
     (slot table at `ctx+360+4*id`, ids capped at 116);
  4. `0xD32E70(ctx, 41, result)` — store the u32 as that slot's result
  ...
  PROTOCOL.md: "Reply 0x4391, result 0 always; validation failures are logged, not
  surfaced." Live-exercised on every round end (14 reports traced 2026-07-23).
...
seq:
  - id: result
    type: s4
    doc: |
      [ELF 0xd4077c] The only field. 0 = success. Signed: the client stores it with `lwa`
