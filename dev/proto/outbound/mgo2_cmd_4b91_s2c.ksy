meta:
  id: mgo2_cmd_4b91_s2c
  title: "MGO2 0x4b91 — clan search START (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Start of the clan search results.** First packet of the 0x4b91 / 0x4b92 / 0x4b93 triple
  answering 0x4b90, `{u8 exact_only, u8 ignore_case, name[16]}`. [CONFIRMED LIVE 2026-07-27].

  The two request bytes are the search screen's own toggles: "Partial and Exact Matches" vs "Exact
  Matches Only", and "Case Sensitive" vs "Case Insensitive". **1 in the second byte means IGNORE
  case**, which is the opposite of how it was first read — settled live when "bob" failed to find
  "Bob" on the sibling player search, which receives the same two bytes from its own toggles.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD54DC4**,
  which re-checks the id (`cmpwi r0,19345` = 0x4b91) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 114
  ...
  Special case: result -611 (0xFFFFFD9D) takes the *success* path anyway (0xD54E8C) —
  the client treats it as "empty list", arms the gate and zeroes the count.
  List-triple: the item records arrive as 0x4b92; this packet is the paired start/end.
  Gate word at session[+0x10000+6404] + 0x20000 + 10344: must be 0 on arrival; the parser arms it and zeroes the record count at +0x04.
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Search-start result. 0 = the rows follow. **A result code, never a
      count** — see the top-level doc. -611 takes the success path anyway (empty list).
