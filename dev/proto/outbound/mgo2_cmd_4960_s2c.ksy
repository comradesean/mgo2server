meta:
  ...
  title: "MGO2 0x4960 — server -> client: team notification carrying one 4-byte word (key check waived)"
  endian: be
  encoding: ISO-8859-1
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (compare tree at `0xD38804`) matches `cmpwi 0x4960` at `0xd38c1c` and branches to the
  ...
  client's cached team record disagrees. Post-processing locates the member slot whose first
  ...
  **RELABELLED 2026-08-01 — the header pair is the TEAM id and the TEAM record serial, not a
  ...
  record: this parser re-bases it with `addis` +1 / `addi ...,-9944` at [ELF 0xd4c8dc] and passes
