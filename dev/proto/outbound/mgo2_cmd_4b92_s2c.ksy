meta:
  ...
  title: "MGO2 0x4b92 — clan SEARCH results, 44-byte records (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Clan search results.** Middle packet of the 0x4b91 / 0x4b92 / 0x4b93 triple answering 0x4b90,
  `{u8 exact_only, u8 ignore_case, name[16]}`. [CONFIRMED LIVE 2026-07-27] — searching by name
  returns rows that render with the clan name, the leader's name and the founding date, and picking
  one leads to 0x4b80 / 0x4b81.

  **The record is 44 bytes.** Another server implementation writes 48 here, adding a flag byte and
  three pad bytes before the trailing u32. That is a different client build, and per CLAUDE.md
  another implementation is a **tier-4 source and not a specification**: this parser reads 44
  (0xD55BB0 loop, `lswi/stswi` at 0xD55C78 copying 32 + 28 into a 60-byte struct), and the parser is
  tier 1. 44 it is; the two builds differ.
...
types:
  record:
    doc: "[CONFIRMED 2026-07-27] 44 wire bytes -> 60-byte client struct. One matching clan."
    seq:
      - id: clan_id
        type: u4
        doc: "[CONFIRMED 2026-07-27] struct+0x00, the clan's id — what 0x4b80 is sent with when the row is picked."
      - id: name
        size: 16
        type: str
        encoding: ASCII
        doc: "[CONFIRMED 2026-07-27] struct+0x04, the clan's name, 16 bytes fixed."
      - id: leader_chara_id
        type: u4
        doc: "[CONFIRMED 2026-07-27] struct+0x18, the leader's CHARACTER id (not another clan id)."
      - id: leader_name
        size: 16
        type: str
        encoding: ASCII
        doc: "[CONFIRMED 2026-07-27] struct+0x1c, the leader's name, 16 bytes fixed."
      - id: founded_at
        type: u4
        doc: |
          [CONFIRMED 2026-07-27] struct+0x30, the clan's founding date in Unix seconds, and the last
          4 bytes of the record.

          This is the field the 48-byte record another implementation writes pushes four bytes
          later, behind a flag byte and three pads. Not this build — see the top-level doc.
