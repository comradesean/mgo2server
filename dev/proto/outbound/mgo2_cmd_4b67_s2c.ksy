meta:
  id: mgo2_cmd_4b67_s2c
  title: "MGO2 0x4b67 — set clan notice result (server -> client)"
  endian: be
doc: |
  Decrypted payload after the 24-byte transport header (dev/docs/CRYPTO.md).

  **Set the clan notice.** The reply to 0x4b66, a 512-byte fixed-width text write.
  [CONFIRMED LIVE 2026-07-27]: setting **Clan Notice** to "watching you" on the client sent exactly
  this, 512 bytes. That is what identifies 0x4b21's T+0x700 as the notice, which this spec's
  sibling had down as an unknown "long text block or packed table".

  The notice's timestamp (0x4b21 T+0x904) and its author's name (T+0x908) are rendered under it.

  Routing: GAME dispatcher 0xD387C8, compare tree at 0xD38804 -> thunk -> parser **0xD568FC**,
  which re-checks the id (`cmpwi r0,19303` = 0x4b67) before reading anything.

  Whole payload is ONE s4. The parser reads it, closes pending-request slot 110
...
seq:
  - id: result
    type: s4
    doc: |
      [CONFIRMED 2026-07-27] Set-notice result. 0 = the notice was stored.
