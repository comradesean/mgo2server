meta:
  id: mgo2_cmd_4942_s2c
  title: "MGO2 0x4942 — server -> client: team notification carrying one 4-byte word"
  endian: be
  encoding: ISO-8859-1
doc: |
  Evidence: GAME reply dispatcher `0xD387C8` (compare tree at `0xD38804`) matches `cmpwi 0x4942` at `0xd38c68` and branches to the
  thunk at `0xd39760`, which tail-calls the parser at `0xd4cc80`. Channel A (lobby TCP).
  ...
  request-status slot. The parser ends by raising client event **6** via `0xD33CD8(ctx, 6)`.
  ...
  Header, then a single 4-byte read, bounds-tested against 7 as a member index
  [READ 0xd4cd64]. Event 6 is raised, then the local team state is reset (`0xD49368` /
  `0xD4EC14`) and a 28-byte member slot is zeroed [READ 0xd4cdb0-0xd4cdd8]. Payload 10 bytes.
  ...
  **RELABELLED 2026-08-01 — the header pair is the TEAM id and the TEAM record serial, not a
  ...
  record: this parser re-bases it with `addis` +1 / `addi ...,-9944` at [ELF 0xd4ccec] and passes
...
seq:
  ...
  - id: member_slot
    type: u4
    doc: "[INFERRED] member array index; > 7 skips the mutation [ELF 0xd4cd64]."
