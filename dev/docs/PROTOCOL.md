...
| B24 | TDM only (0 across every DM round incl. wins); 1,2 within a stage then reset (log-confirmed rotations — NOT an unbounded streak); quit-teardown snapshots the pre-round value; survive-but-lose did NOT tick it (game 127 R2) and win-but-die did NOT tick it (game 131 R2) — the event is exactly `0x21` | **flawless TDM wins within the stage** — whether it is the *count* or the *best consecutive run* of them is untested: every observed stage was 2 rounds, where the two coincide (they diverge only in a ≥3-round stage with a flawless/non-flawless/flawless pattern). The slot-25 label "Consecutive Survivals" favours the streak reading |
