...
| — | **The actual auto-patch install blocker, found live 2026-07-31: the client never calls `mkdir`, anywhere.** The install loop (`0xBB8E6C`-`0xBB903C`) is `sprintf path → open(dev 1, RD) → open(dev 7, O_CREAT\|O_WRONLY) → read → write → close ×2 → unlink`. `O_CREAT` creates the file, not missing parent directories, so with no `dl` folder on disk at all the device-7 open returns `ENOENT` and the loop bails silently at `0xBB93D0` — no crash, no further network request, matching exactly what was observed against a real client. Creating the empty tree `USRDIR/dl/p/ar/t/0/` (TOC strings `dl/p/`, `dl/p/ar/`, `dl/p/ar/t/0/`) is sufficient; no archive and no `dl/.p`/`dl/p/.l` are required for the install *write* to succeed — those would only matter for the client to later *read back* an installed file, which this binary cannot do unassisted regardless |
