...
**four are reachable in ordinary play** and are therefore live `FFFFFF60` candidates —

| id | dir | status |
| --- | --- | --- |
| `0x4210` | c2s | own player card / overview request |
| `0x4348` | c2s | unidentified in-match command |
| `0x4394` | c2s | in-match large record push, 203 bytes |
| `0x43B0` | c2s | in-match eight-field report, 29 bytes |

**All four already have real ELF-derived layouts** in `dev/proto/`, so they are implementable now
